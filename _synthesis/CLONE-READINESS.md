...
## Airbnb — 🟢 클론 레벨 도달 (APK GraphQL 카탈로그 추가)

| 영역 | 판정 | 근거 |
|---|---|---|
| **GraphQL API 표면** | 🟢 | `graphql-api.md` — **1,363 오퍼레이션**(Q 751·M 612) dex 확정 |
| **요청 타입** | 🟢 | `data-model.md` §6 — **566 Input 타입** |
| **응답 스키마** | 🟡 | 웹 `niobeClientData`로 핵심(PDP·검색·체크아웃·위시리스트·계정·여행) 확보. 나머지는 오퍼레이션명+Relay 패턴으로 유추 |
...
| **호스트 심화** | 🟡 | 리스팅123·캘린더53·정산17 오퍼레이션 확보, 실화면 일부(등록숙소 0건) |

**종합 ~85%.** persisted query 특성상 개별 오퍼레이션의 전체 필드 셀렉션은 웹 관찰분만 완전하지만,
**오퍼레이션 카탈로그+Input 타입+enum+플로우+화면**이 확보되어 기능 구조를 동일하게 구현 가능한 수준.

> ⚠️ 남은 한계: 오퍼레이션별 전체 응답 필드(웹 미방문 화면), persisted hash(배포 변동), 서버 로직.
> Airbnb는 **참고 대상**이므로 이 수준이면 기능 벤치마킹+선택적 클론에 충분.
