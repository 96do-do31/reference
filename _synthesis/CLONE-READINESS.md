...
| **레이아웃 스펙** | 🟢 | 모바일 XML bounds(px) + `responsive-web.md`(웹 UA쌍 실측) | 태블릿 중간폭 근사만 |
...
- 레이아웃: 모바일 XML px 좌표 20개 화면 + **웹 반응형 트리 실측**(`responsive-web.md`, UA쌍 6페어)
- 반응형 전략 확정: 33m2=서버 UA 적응형 이중 트리(데스크톱 고정 1080px / 모바일 유동+하단 5탭바), Airbnb=클라이언트 CSS 반응형(744/950/1128)
- 🟡 **태블릿 중간폭(768~1024) 픽셀 세부**만 `md:` 클래스로 근사. "유사" 기준 충족, "픽셀 동일"은 스크린샷 대조 필요.
